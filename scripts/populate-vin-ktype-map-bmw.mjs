#!/usr/bin/env node
/**
 * Re-populate vin_ktype_map for BMW only, using the improved TecDoc resolver.
 * This fixes the previous bad mappings where 520d → BMW 1 series etc.
 */
...
const OUTPUT_SQL = path.join(DATA_DIR, "vin-ktype-map-bmw-inserts.sql");
const REPORT_FILE = path.join(DATA_DIR, "vin-ktype-map-bmw-report.json");
  ...
  for (const fn of INPUT_FILES) {
    ...
    for await (const line of rl) {
      ...
      try {
        ...
        if ((d.make || "").toUpperCase() !== "BMW") continue;
        yield {
          regnr: d.regnr || "",
          make: d.make || "",
          model: d.model || "",
...
async function main() {
  log("Starting BMW VIN → kType re-population with improved resolver");
  ...
  let batchValues = [];
  let batchNumber = 0;
  fs.writeFileSync(OUTPUT_SQL, "-- BMW VIN → kType mappings (improved resolver)\n");
  ...
  // Apply SQL via wrangler (use absolute path so cwd does not matter)
  ...
  log(`BMW mapping complete: ${matched}/${processed} (${matchRate}%)`);
