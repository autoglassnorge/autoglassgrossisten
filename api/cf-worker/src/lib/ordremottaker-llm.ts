...
export async function generateDialogue(
  ...
  const systemPrompt =
    ...
    `- Features: CS=coated (EU), SOLAR=coated (US-biler), COLD=bakrute uten varmetråder, EL=varmetråder, M=regnsensor, ENC=innkapslet, CSL=coated+laminert, U=HUD, T=tollvisir, ANT=antenne, AKU=akustisk, LDW/HUD=ADAS, LG=venstre/RG=høyre dørglass (BMW), siffer=generasjonsvariant\n` +
